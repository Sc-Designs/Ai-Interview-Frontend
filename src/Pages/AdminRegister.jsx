...
const AdminRegister = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const res = await adminAxios.post("/admin/api/register", data);
      if(res.status === 201){
        localStorage.setItem("AdminemailForOtp", data.email);
        toast.success("OTP sent to your email. Please verify.");
        navigate("/admin-otp");
      } else if (res.status === 406){
        toast.info("Admin already existing, Please Login.");
        navigate("/admin-login");
      } else if (res.status === 403){
        toast.warning("⚠️ Admin limit reached. Please remove an existing admin to proceed.");
        navigate("/admin-login");
      }
    } catch (err){
    ...
    <div className="flex items-center justify-center min-h-screen px-4 bg-black">
      <div className="w-full max-w-md px-4 py-8 lg:p-8 bg-zinc-900/60 border-1 border-zinc-200/20 rounded-2xl font-Satoshi">
        ...
        <p className="mb-6 text-center text-gray-500">
          Create your account Admin to check activity
        </p>
